...
const PRIVILEGE_ESCALATION_BLOCKLIST: readonly string[] = [
...
function isPrivilegeEscalation(command: string): boolean {
...
function detectInjectionInOutput(text: string): { detected: boolean; patterns: string[] } {
...
suite('Security Tests', () => {
        ...
        suite('Prompt Sanitization', () => {
                ...
                test('should remove null bytes from prompts', () => {
                        ...
                        assert.strictEqual(sanitise(null as any), '');
                        assert.strictEqual(sanitise(undefined as any), '');
                });
        ...
        suite('Rate Limiting', () => {
                ...
                test('should reset rate limit after window', () => {
                        ...
                        (limiter as any).commandTimestamps = [Date.now() - TERMINAL_RATE_LIMIT.windowMs - 1000];
