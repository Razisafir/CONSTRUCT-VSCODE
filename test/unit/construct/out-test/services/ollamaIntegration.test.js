...
suite('Ollama Integration Tests', function () {
    ...
    suiteSetup(async function () {
        try {
            ...
            if (res.ok) {
                ...
                ollamaAvailable = models.some((name) => name === OLLAMA_MODEL || name.startsWith(OLLAMA_MODEL + ':'));
                if (!ollamaAvailable) {
    ...
    test('Ollama provider can list models', async function () {
        ...
        const data = await res.json();
        const models = (data.models || []).map((m) => m.name || m);
        const match = models.some((name) => name === OLLAMA_MODEL || name.startsWith(OLLAMA_MODEL + ':'));
    ...
    test('Ollama provider handles multi-turn conversation', async function () {
        ...
        assert.ok(content.includes('testagent') || content.includes('test agent'), `Response should mention "TestAgent", got: ${data2.message?.content?.substring(0, 200)}`);
    });
    ...
    test('Ollama provider handles invalid model gracefully', async function () {
        ...
        assert.ok(text.toLowerCase().includes('not found') ||
            ...
            res.status === 404, `Error should mention "not found" or be 404, got status ${res.status}: ${text.substring(0, 200)}`);
    });
    test('Ollama provider respects max_tokens parameter', async function () {
        ...
        const data1 = await res1.json();
        const shortContent = data1.message?.content || '';
        // Unconstrained
        const res2 = await fetch(`${OLLAMA_HOST}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const data2 = await res2.json();
        const longContent = data2.message?.content || '';
        assert.ok(shortContent.length <= longContent.length + 50, `Constrained (${shortContent.length} chars) should be shorter than or similar to unconstrained (${longContent.length} chars)`);
    });
});
//# sourceMappingURL=ollamaIntegration.test.js.map