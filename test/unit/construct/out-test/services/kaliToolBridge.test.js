...
class KaliAuditLogger {
    ...
    getEntriesByTool(tool) {
        return this.entries.filter(e => e.tool === tool);
    }
}
// ---- Tests ----
suite('KaliToolBridge', () => {
    const nativeDist = { type: 'native', name: 'Kali Linux', available: true };
    const wslDist = { type: 'wsl', name: 'Kali Linux (WSL)', available: true };
    const dockerDist = { type: 'docker', name: 'Kali Linux (Docker)', available: true };
    const unavailableDist = { type: 'native', name: 'Kali Linux', available: false };
    ...
    suite('Rate limiting — max 5 concurrent', () => {
        ...
        test('rejects execution when at max capacity', () => {
            ...
            assert.throws(() => limiter.startExecution(), /Maximum 5 concurrent/);
        });
...
//# sourceMappingURL=kaliToolBridge.test.js.map