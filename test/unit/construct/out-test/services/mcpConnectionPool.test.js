...
class MCPPermissionEnforcer {
    ...
    serverName;
    _auditLog = [];
    constructor(config, serverName = '<unknown>') {
        this.config = config;
        this.serverName = serverName;
    }
    validateFileAccess(path, mode) {
 ...
 */
function buildMinimalEnv(serverEnv, platform = 'linux') {
    const minimalEnv = {
        PATH: '/usr/bin:/bin',
        HOME: '/home/user',
        USER: 'user',
 ...
 */
function envContainsProcessEnv(env, processEnvKeys) {
    const leaks = [];
...
class SimpleConnectionPool {
    ...
    executeTool(serverName, toolName, args) {
        ...
        if (args.path && !entry.enforcer.validateFileAccess(args.path, 'read')) {
...
suite('MCPConnectionPool', () => {
    suite('Server lifecycle — start/stop', () => {
        ...
        test('pool enforces maximum concurrent servers', () => {
            ...
            assert.throws(() => pool.addServer('one-too-many', { level: 'untrusted' }), /Connection pool full/);
        });
...
//# sourceMappingURL=mcpConnectionPool.test.js.map