...
class ConfigurationService {
        ...
        private schema: Map<string, IConfigSchema> = new Map();
        private changeListeners: ((event: IConfigChangeEvent) => void)[] = [];
        private changeLog: IConfigChangeEvent[] = [];
        ...
        set(key: string, value: unknown, scope: 'global' | 'workspace' = 'workspace'): string[] {
                const s = this.schema.get(key);
                if (!s) { return [`Unknown config key: ${key}`]; }
                ...
                const event: IConfigChangeEvent = { key, oldValue, newValue: value, scope };
        ...
        private validateValue(key: string, value: unknown): string[] {
                const s = this.schema.get(key);
                if (!s) { return [`Unknown config key: ${key}`]; }
        ...
        getChangeLog(): IConfigChangeEvent[] { return [...this.changeLog]; }
        ...
        mergeWithDefaults(userConfig: Record<string, unknown>): Record<string, unknown> {
        ...
        migrate(oldVersion: string, config: Record<string, unknown>): Record<string, unknown> {
...
suite('Configuration Tests', () => {
        ...
        suite('Defaults and Merging', () => {
                test('should load default configuration', () => {
                        ...
                        assert.strictEqual(config.get<string>('construct.agent.model'), 'gpt-4');
                        assert.strictEqual(config.get<number>('construct.agent.maxTokens'), 4096);
                        assert.strictEqual(config.get<boolean>('construct.terminal.restrictedMode'), false);
                        assert.strictEqual(config.get<number>('construct.terminal.commandTimeout'), 60);
                });
