...
interface IMemoryEntry {
	...
	category: string;
	timestamp: number;
	expiration?: number; // Unix timestamp, undefined = never expires
	tags: string[];
	metadata?: Record<string, unknown>;
}

class MemoryStore {
	private entries: Map<string, IMemoryEntry> = new Map();
	...
	retrieve(id: string): IMemoryEntry | undefined {
		return this.entries.get(id);
	}

	search(query: string): IMemoryEntry[] {
	...
	fuzzySearch(query: string, threshold = 0.3): IMemoryEntry[] {
	...
	getByCategory(category: string): IMemoryEntry[] {
	...
	compact(): number {
		...
		const byContent = new Map<string, IMemoryEntry>();
		for (const entry of this.entries.values()) {
	...
	private evictOldest(): void {
		let oldest: IMemoryEntry | undefined;
		for (const entry of this.entries.values()) {
...
suite('Memory System Tests', () => {
	...
	suite('Store and Retrieve', () => {
		...
		test('should persist memory across sessions', () => {
			...
			const restored: IMemoryEntry = safeJsonParse(serialized, null as unknown as IMemoryEntry);
