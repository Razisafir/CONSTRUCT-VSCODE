...
exports.IWorkspaceContextService = (0, instantiation_js_1.createDecorator)('contextService');
function isSingleFolderWorkspaceIdentifier(obj) {
    const singleFolderIdentifier = obj;
    return typeof singleFolderIdentifier?.id === 'string' && uri_js_1.URI.isUri(singleFolderIdentifier.uri);
}
...
function isWorkspaceIdentifier(obj) {
    ...
    return typeof workspaceIdentifier?.id === 'string' && uri_js_1.URI.isUri(workspaceIdentifier.configPath);
}
function reviveIdentifier(identifier) {
    ...
    if (singleFolderIdentifierCandidate?.uri) {
        return { id: singleFolderIdentifierCandidate.id, uri: uri_js_1.URI.revive(singleFolderIdentifierCandidate.uri) };
    }
    // Multi folder
    const workspaceIdentifierCandidate = identifier;
    if (workspaceIdentifierCandidate?.configPath) {
        return { id: workspaceIdentifierCandidate.id, configPath: uri_js_1.URI.revive(workspaceIdentifierCandidate.configPath) };
    }
    // Empty
    if (identifier?.id) {
...
function isWorkspaceFolder(thing) {
    const candidate = thing;
    return !!(candidate && typeof candidate === 'object'
        && uri_js_1.URI.isUri(candidate.uri)
...
class Workspace {
    ...
    _configuration;
    _ignorePathCasing;
    _foldersMap = ternarySearchTree_js_1.TernarySearchTree.forUris(this._ignorePathCasing, () => true);
    _folders;
    ...
    updateFoldersMap() {
        this._foldersMap = ternarySearchTree_js_1.TernarySearchTree.forUris(this._ignorePathCasing, () => true);
...
class WorkspaceFolder {
    ...
    toResource(relativePath) {
        return (0, resources_js_1.joinPath)(this.uri, relativePath);
    }
    toJSON() {
...
function toWorkspaceFolder(resource) {
    return new WorkspaceFolder({ uri: resource, index: 0, name: (0, resources_js_1.basenameOrAuthority)(resource) }, { uri: resource.toString() });
}
exports.WORKSPACE_EXTENSION = 'code-workspace';
exports.WORKSPACE_SUFFIX = `.${exports.WORKSPACE_EXTENSION}`;
exports.WORKSPACE_FILTER = [{ name: (0, nls_js_1.localize)('codeWorkspace', "Code Workspace"), extensions: [exports.WORKSPACE_EXTENSION] }];
exports.UNTITLED_WORKSPACE_NAME = 'workspace.json';
function isUntitledWorkspace(path, environmentService) {
    return resources_js_1.extUriBiasedIgnorePathCase.isEqualOrParent(path, environmentService.untitledWorkspacesHome);
}
function isTemporaryWorkspace(arg1) {
    ...
    return path?.scheme === network_js_1.Schemas.tmp;
}
...
function hasWorkspaceFileExtension(path) {
    const ext = (typeof path === 'string') ? (0, path_js_1.extname)(path) : (0, resources_js_1.extname)(path);
