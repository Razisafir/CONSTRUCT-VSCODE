...
export class KovixSettingsMigrationContribution extends Disposable implements IWorkbenchContribution {
	...
	private async migrateSettingsFile(settingsUri: URI): Promise<number> {
		...
		await this.fileService.writeFile(backupUri, content);
		...
		await this.fileService.writeFile(settingsUri, newContent);
		return migrated;
	}
	...
	private async migrateKeybindingsFile(keybindingsUri: URI): Promise<number> {
		...
		const newContent = JSON.stringify(parsed, null, '\t') + '\n';
		await this.fileService.writeFile(keybindingsUri, newContent);
		return migrated;
	}

	/**
	 * Minimal JSON comment stripper — handles // line comments and /* block */
	 * comments. VS Code's settings.json and keybindings.json both allow
	 * comments, but the built-in JSON.parse does not. We avoid pulling in the
	 * full jsonc-parser for this one-shot migration.
	 *
	 * Limitations: does not handle comments inside string literals. That's
	 * acceptable here because user settings rarely contain `//` inside string
	 * values, and if they do, the migration will simply log a warning and
	 * skip the file (the backup preserves the original).
	 */
