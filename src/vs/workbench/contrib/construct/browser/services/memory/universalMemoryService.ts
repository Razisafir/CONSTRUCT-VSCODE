...
export class UniversalMemoryService extends Disposable implements IUniversalMemoryService {
        ...
        async getStats(): Promise<IUniversalMemoryStats> {
                ...
                try {
                        ...
                        this.logService.debug('[UniversalMemory] Storage file stat failed (may not exist yet): ' + (statErr instanceof Error ? statErr.message : String(statErr)));
                }
        ...
        private async loadStore(): Promise<IUniversalMemoryStore> {
                ...
                try {
                        ...
                        this.logService.debug('[UniversalMemory] Store file load failed, starting fresh: ' + (loadErr instanceof Error ? loadErr.message : String(loadErr)));
                }
        ...
        private async getStorageFilePath(): Promise<string | null> {
                try {
                        ...
                        this.logService.debug('[UniversalMemory] Cannot determine home directory: ' + (osErr instanceof Error ? osErr.message : String(osErr)));
                        return null;
                }
        }
        ...
        private async getFs(): Promise<typeof import('fs') | null> {
                try {
                        ...
                        this.logService.debug('[UniversalMemory] Cannot import fs module: ' + (fsErr instanceof Error ? fsErr.message : String(fsErr)));
                        return null;
                }
        }
