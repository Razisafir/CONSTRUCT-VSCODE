...
class ConstObservable extends base_js_1.ConvenientObservable {
    value;
    constructor(value) {
        super();
        this.value = value;
...
function observableFromPromise(promise) {
    const observable = (0, base_js_1.observableValue)('promiseValue', {});
    promise.then((value) => {
...
function observableFromEvent(...args) {
    ...
    return new FromEventObservable(new debugName_js_1.DebugNameData(owner, undefined, getValue), event, getValue, () => FromEventObservable.globalTransaction, deps_js_1.strictEquals);
}
function observableFromEventOpts(options, event, getValue) {
    return new FromEventObservable(new debugName_js_1.DebugNameData(options.owner, options.debugName, options.debugReferenceFn ?? getValue), event, getValue, () => FromEventObservable.globalTransaction, options.equalsFn ?? deps_js_1.strictEquals);
}
class FromEventObservable extends base_js_1.BaseObservable {
    _debugNameData;
    event;
    _getValue;
    _getTransaction;
    ...
    handleEvent = (args) => {
        ...
        if (didChange) {
            ...
            if (this.hasValue) {
                ...
                (0, base_js_1.subtransaction)(this._getTransaction(), (tx) => {
                    (0, logging_js_1.getLogger)()?.handleFromEventObservableTriggered(this, { oldValue, newValue, change: undefined, didChange, hadValue: this.hasValue });
...
class FromEventObservableSignal extends base_js_1.BaseObservable {
    debugName;
    event;
    subscription;
...
class ObservableSignal extends base_js_1.BaseObservable {
    _debugName;
    _owner;
    get debugName() {
...
function debouncedObservable(observable, debounceMs, disposableStore) {
    const debouncedObservable = (0, base_js_1.observableValue)('debounced', undefined);
    let timeout = undefined;
    disposableStore.add((0, autorun_js_1.autorun)(reader => {
...
function wasEventTriggeredRecently(event, timeoutMs, disposableStore) {
    const observable = (0, base_js_1.observableValue)('triggeredRecently', false);
    let timeout = undefined;
...
(0, base_js_1._setKeepObserved)(keepObserved);
/**
...
(0, base_js_1._setRecomputeInitiallyAndOnChange)(recomputeInitiallyAndOnChange);
